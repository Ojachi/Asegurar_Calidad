...
const RiskMatrix = () => {
    ...
    const calculateRisk = (probability, impact) => {
        ...
        else if (probabilityImpact >= 8 && probabilityImpact <= 13) levelRisk = 'Medio';
        else if (probabilityImpact >= 14 && probabilityImpact <= 19) levelRisk = 'Alto';
        else if (probabilityImpact >= 20 && probabilityImpact <= 25) levelRisk = 'Muy Alto';
    ...
    const handleAddRisk = () => {
        if (!newRisk.description || !newRisk.fase || !newRisk.probability || !newRisk.impact) {
    ...
    return (
        <div className="risk-matrix-container">
            ...
            <button onClick={handleSubmit}>Enviar Matriz de Riesgos</button>
        </div>
    );
};
