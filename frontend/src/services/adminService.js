...
export const addQuestion = async (text) => {
    try {
        const token = getToken();
        await axios.post(`${API_URL}/questions`, { text }, {
            headers: { Authorization: `Bearer ${token}` },
        });
    } catch (error) {
        console.error('Error al agregar pregunta:', error.response?.data || error.message);
        throw error;
    }
};
...
export const deleteQuestion = async (questionId) => {
    try {
        ...
        console.error('Error al eliminar pregunta:', error.response?.data || error.message);
        throw error;
    }
};
