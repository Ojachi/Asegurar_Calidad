...
def register():
    ...
    data = request.json
    if not data or not all(k in data for k in ('username', 'email', 'password', 'cedula')):
        return response("Datos incompletos", 400)

    token = register_user(data['username'], data['email'],
...
def login():
    ...
    token, user_data = login_user(data['cedula'], data['password'])
    if not token:
        return response("Credenciales incorrectas", 401)

    return response("Inicio de sesión exitoso", 200, {'token': token, 'role': user_data['role']})
